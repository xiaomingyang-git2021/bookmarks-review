...
const createBookmark = async (bookmark)=>{
  try{
    const newBookmark = await db.one(
      "INSERT INTO bookmarks (name, url, category, is_favorite) VALUES($1, $2, $3, $4) RETURNING *", [bookmark.name, bookmark.url, bookmark.category, bookmark.is_favorite]
    )
    return newBookmark;
  } catch(err){
    return err;
...
const updateBookmark = async(id, bookmark)=>{
  try{
    const updatedBookmark = await db.one(
      ...
      [bookmar.name, bookmark.url, bookmark.category, bookmark.is_favorite, id]
    )
